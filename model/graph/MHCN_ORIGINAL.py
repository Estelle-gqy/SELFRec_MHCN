...
class MHCN(GraphRecommender):
    def __init__(self, conf, training_set, test_set, **kwargs):
        GraphRecommender.__init__(self, conf, training_set, test_set, **kwargs)  # 得到self.data
    ...
    # 这个其实不是tensorflow的内置函数，及重写父类Recommender中的build函数，且该函数在execute()函数中调用，
    # 所以在SELFRec.py的最后一行中运行了execute函数
    ...
    def build(self):
        ...
        with tf.name_scope('self_gating'):
        ...
        with tf.name_scope('multi_channel_conv'):
        ...
        with tf.name_scope('avg_multi_channel_conv'):
        ...
        with tf.name_scope('aggregate_multi_channel_conv'):
        ...
        with tf.name_scope('self_supervised_loss'):
