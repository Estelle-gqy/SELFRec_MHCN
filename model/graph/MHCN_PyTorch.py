...
class MHCN(GraphRecommender):
	def __init__(self, conf, training_set, test_set, dev_data, **kwargs):
		...
		self.model = Hypergraph_Rec(self.n_layers, self.ss_rate, self.social_data)
	...
	def train(self):
		...
		for epoch in range(self.maxEpoch):
			for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
				...
				outputxx = model()  # 模型输入以及输出和forward函数对应
...
class Hypergraph_Rec(nn.Module):
	def __init__(self, n_layers, ss_rate, social_data):
		pass
